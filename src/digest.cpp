  ...
  while (std::getline(input, line)) {
    ...
    try {
      ...
      if (is_equal) {
        ...
        if (istr.peek() == '=') {
          istr.ignore();
          if (is_eq_zero(nutr.wt) && is_eq_zero(nutr.vol)) {
      ...
      if (!is_equal && key.empty()) {
        ...
        if (is_lteq_zero(nutr.energy)) {
          nutr.energy = energy;
        }
        else {
          auto err = static_cast<double>
                     (mp_units::abs(energy - nutr.energy) / nutr.energy);
      ...
      if (!key.empty()) {
        ...
        if (is_gteq_zero(nutr.energy)) {
          ...
          else if (is_neq_zero(nutr.wt) && is_neq_zero(n.wt))
            scale = static_cast<double>(nutr.wt / mp_units::abs(n.wt));
          else if (is_neq_zero(nutr.vol) && is_neq_zero(n.vol))
            scale = static_cast<double>(nutr.vol / n.vol);
          if (scale == 0.0) {
          ...
          if (is_eq_zero(nutr.energy))
            nutr.energy = scale * n.energy;
          if (is_eq_zero(nutr.wt))
            nutr.wt = scale * mp_units::abs(n.wt);
          if (is_eq_zero(nutr.vol))
      ...
      if (is_eq_zero(nutr.wt)) {
        if (is_eq_zero(nutr.vol) && !allow_each)
          COUT << "allow each assumed\n";
      }
      else {
        nutr.wt = allow_each ? -mp_units::abs(nutr.wt) : mp_units::abs(nutr.wt);
      }
  #if 0
